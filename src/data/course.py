...
from config import CUSTOM_VTK_URL_MAP
...
class Course:
    def __init__(self, config : Config, name, id) -> None:
        self.name = name
        self.id = id
    ...
    async def load_toc(self, session : ClientSession):
        ...
        if r.status == 200:
            self.toc = [Module(self.config, self.id, module_dict) for module_dict in json.loads(await r.read())['Modules']]
    ...
    @property
    def vtk_url(self):
        return CUSTOM_VTK_URL_MAP.get(self.id, f'https://vtk.ugent.be/wiki/{ self.display_name.replace(" ", "-") }')


    def get_toc_html(self):
        content = ''.join([m.get_html() for m in self.toc])
        return JINJA_ENV.get_template("toc.html").render(display_name=self.display_name, content=content)

    def get_announcements_html(self):
        return "".join([a.get_html() for a in self.announcements])

    def get_html(self):
        return JINJA_ENV.get_template("course.html").render(
            id=self.id, 
    ...
    def get_bottom_html(self):
        return JINJA_ENV.get_template("bottom_links.html").render(
            id=self.id,
