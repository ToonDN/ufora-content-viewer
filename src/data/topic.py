...
class Topic:
    ...
    def usable_url(self):
        if self.is_downloaded:
            return self.config.downloads_dir + '/' + [name for name in os.listdir(self.config.downloads_dir) if str(self.full_id) in name][0]
    ...
    def is_downloaded(self):
        return str(self.full_id) in [name.split(".")[0] for name in os.listdir(self.config.downloads_dir)]
    ...
    async def convert_and_download(self, session : ClientSession, convert_mapping : dict, download_list : tuple):
        if not self.is_downloaded:
            if self.type in convert_mapping.keys():
                ...
                # Define filenames
                temp_file = f'{self.config.temp_dir}/{self.full_id}.{in_ft}'
                final_file = f'{self.config.downloads_dir}/{self.full_id}.{out_ft}'
                ...
                if in_ft in ("ppt", "pptx") and out_ft == "pdf":
                    os.system(f"libreoffice --headless --invisible --convert-to pdf {temp_file} --outdir {self.config.downloads_dir} && rm {temp_file}")
                ...
                ft = self.type
                # Define filenames
                temp_file = f'{self.config.temp_dir}/{self.full_id}.{ft}'
                final_file = f'{self.config.downloads_dir}/{self.full_id}.{ft}'
                print(f"--- Downloading {self.title}")
    ...
    def get_html(self):
        return JINJA_ENV.get_template("topic_tile.html").render(
            url=self.usable_url, 
            title=self.title, 
            type=self.type, 
